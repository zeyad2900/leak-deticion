    ...
    <div class="fixed top-0 left-0 w-full bg-[#0000009a] flex items-center justify-center h-screen z-[1002]">
        <div class="bg-white rounded-2xl md:w-1/2 w-[90%] p-8">
            ...
            <div class="relative">
                ...
                >
                    <GMapMarker :position="center" :clickable="true" :draggable="true" @dragend="updateMarker"> </GMapMarker>
                </GMapMap>
...
const marker = ref({ lat: 30.033333, lng: 31.233334 });
...
watch(
    ...
    async (value) => {
        if (value.lat && value.lng) {
            const { data: res } = await useFetch(`https://maps.googleapis.com/maps/api/geocode/json?latlng=${value?.lat},${value?.lng}&key=AIzaSyDRymdCLWxCwLHFnwv36iieKAMjiwk8sdc`);

            enteredAddress.value = res.value.results[0].formatted_address;
