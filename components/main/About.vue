        ...
        <div class="grid grid-cols-3 gap-9">
            <div class="col-span-3 space-y-5 lg:col-span-2 order-2 lg:order-1">
                ...
                <div class="grid sm:grid-cols-2 gap-4">
                    <div v-for="advantage in advantages" :key="advantage.id" class="flex gap-3 items-start">
                        <nuxt-img :src="advantage.image" placeholder="/car.png" width="22" height="22" alt="truck fast" class="mt-2" />
                        <div>
                            <h3 class="font-medium">{{ advantage.title }}</h3>
                            <p class="text-text-light text-sm" v-html="advantage.desc"></p>
                        </div>
                    </div>
                </div>
            ...
            <div class="col-span-3 lg:col-span-1 order-1 lg:order-2 w-[280px] h-[280px] md:w-[400px] md:h-[400px] mx-auto mb-11 lg:mb-0 relative">
                <button @click="myShowAndHideStore.videoHandler('open')" class="round relative">
            ...
            <PopupsVideo v-if="video" item="items.video" />
        </div>
    </article>
</template>
